...
HTML::Microformats::Datatypes::RecurringDateTime - a datetime that recurs

=head1 SYNOPSIS

 my $r_datetime = HTML::Microformats::Datatypes::RecurringDateTime->new($ical_string);
 print "$r_datetime\n";

=cut

package HTML::Microformats::Datatypes::RecurringDateTime;
...
=item C<< $r = HTML::Microformats::Datatypes::RecurringDateTime->new($string, [$context]) >>

Creates a new HTML::Microformats::Datatypes::RecurringDateTime object.
...
=item C<< $r = HTML::Microformats::Datatypes::RecurringDateTime->parse($string, $elem, $context) >>
	...
	foreach my $n ($elem->getElementsByTagName('*'))
	{
		if ($n->getAttribute('class') =~ /\b (until|count) \b/x)
		{
			...
			unless (defined $self->{'until'} || defined $self->{'count'})
			{
				...
				$self->{$p} = HTML::Microformats::Datatypes::DateTime->parse($self->{$p}, $elem, $context)
					if $p eq 'until';
			}
		}
...
=item C<< $r = HTML::Microformats::Datatypes::RecurringDateTime->parse_string($string, [$context]) >>
