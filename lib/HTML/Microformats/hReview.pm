...
HTML::Microformats::hReview inherits from HTML::Microformats::BASE. See the
base class definition for a description of property getter/setter methods,
constructors, etc.

=cut

package HTML::Microformats::hReview;
...
sub new
{
	...
	$self->{'DATA'}->{'rating'} =
		[ HTML::Microformats::hReview::rating->extract_all($clone, $context) ];
...
sub _auto_detect_type
{
	...
	if (UNIVERSAL::isa($self->{'DATA'}->{'item'}, 'HTML::Microformats::hCard')
	...
	elsif (UNIVERSAL::isa($self->{'DATA'}->{'item'}, 'HTML::Microformats::hAudio')
	&&     !defined $self->{'DATA'}->{'type'})
	{
		$self->{'DATA'}->{'type'} = 'product';
	}
	elsif (UNIVERSAL::isa($self->{'DATA'}->{'item'}, 'HTML::Microformats::hEvent')
	&&     !defined $self->{'DATA'}->{'type'})
	{
...
sub add_to_model
{
	...
	{
		if (UNIVERSAL::isa($reviewer, 'HTML::Microformats::hCard'))
		{
...
HTML::Microformats::hReview supports hReview 0.3 and xFolk as described at
