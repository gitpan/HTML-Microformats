...
sub new
{
	...
	$self->_extract_item($clone, 'vcard',  'HTML::Microformats::hCard');
	$self->_extract_item($clone, 'vevent', 'HTML::Microformats::hEvent');
...
sub _hmeasure_parse
{
	...
	{
		my $tolerance = HTML::Microformats::hMeasure->new($nodes[0], $self->context);
...
sub add_to_model
{
	...
	if (ref $item and $item->isa('HTML::Microformats::hCard'))
	{
		$model->add_statement(RDF::Trine::Statement->new(
			$self->id(1),
